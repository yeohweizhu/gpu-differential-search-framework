...
namespace SPN_DIFF{
    ...
    */
    ...
    //This may be done differently for other cipher
    //0 is front, 1 is back [127-64 = 0] [0-63 = 1] (127 is following documentation) 
    // __shared__ unsigned long long perm_lookup_shared[MAX_SBOX][16][2];
    __device__ unsigned long long perm_lookup_global_forward[MAX_SBOX][16][2];
    __device__ unsigned long long perm_lookup_global_reversed[MAX_SBOX][16][2];
    ...
    unsigned long long perm_lookup_host[MAX_SBOX][16][2]; //8192 bytes, 8KB, one SM can have 49KB should be fine
    unsigned long long perm_lookup_host_reversed[MAX_SBOX][16][2];
    ...
    __device__ unsigned int freq_table_global_reversed[][8] = {
        {16, 16, 16, 16, 16, 16, 16, 16},
        {4, 2, 2, 2, 2, 2, 2, 16},
        {4, 2, 2, 2, 2, 2, 2, 16},
        {4, 2, 2, 2, 2, 2, 2, 16},
    ...
    __device__ unsigned int diff_table_size_global_reversed[16] = {1, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7};
    unsigned int diff_table_size_host[16] = {1, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7}; 
    unsigned int diff_table_size_host_reversed[16] = {1, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7}; 
...
GPU_Kenerl_t::GPU_Kenerl_t(int gpu_id, bool is_MITM_used){
    ...
    cudaStatus = cudaMemcpyToSymbol(SPN_DIFF::perm_lookup_global_forward, SPN_DIFF::perm_lookup_host, sizeof(unsigned long long)*32*16*2);
    if (cudaStatus != cudaSuccess) {
    ...
    cudaStatus = cudaMemcpyToSymbol(SPN_DIFF::perm_lookup_global_reversed, SPN_DIFF::perm_lookup_host_reversed, sizeof(unsigned long long)*32*16*2);
    if (cudaStatus != cudaSuccess) {
    ...
    //Input Allocation
    cudaStatus = cudaMalloc((void**)& device_dx, sizeof(unsigned char)* 32 * MAX_PATH_PER_ROUND * MAX_ROUND_FORWARD );
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc device_dx @init failed!");
		goto Error;
    }
    cudaStatus = cudaMemset(device_dx, 0, sizeof(unsigned char)* 32 * MAX_PATH_PER_ROUND * MAX_ROUND_FORWARD);
    if (cudaStatus != cudaSuccess) {
...
__global__ void kernel_diff(
    ...
    if (threadIdx.x <32){
        if (threadIdx.x<16){
            ...
            for (int j = 0; j < 8; j++) {
                ...
                SPN_DIFF::prob_table_shared[threadIdx.x][j] = SPN_DIFF::freq_table_global[threadIdx.x][j]/16.0; 
            }
        }
    ...
    #define output_dx(x) (( device_dx + ( 32 * MAX_PATH_PER_ROUND * (cur_r+1)) + (32 * thread_id_default * MAX_SPACE_PER_THREAD) + (x * 32) )) 
    ...
    //Input
    #define cur_dx() ( ( device_dx + (32 * MAX_PATH_PER_ROUND * cur_r) ) )
    ...
    while(true){ //Base case, cur_0 and reamining == 0
        ...
        for (int i=0;i<loop_limit;i++){
            ...
            if (dx_ptr < MAX_PATH_PER_ROUND){ //If dx_ptr is within dx_num, [0-N)
                ...
                unsigned char* cur_dx_temp = cur_dx() + ( 32 * dx_ptr ) ; //NOTE: Need to modify to fit datastruct of different cipher
                int* cur_sbox_index_ptr = cur_sbox_index() + (MAX_AS  * dx_ptr);
                unsigned char cur_thread_partial_dy[33];
                cur_thread_partial_dy[32] = {0};
                memcpy(cur_thread_partial_dy,cur_dx_temp,32);
                ...
                if (cur_r+1 != MAX_ROUND_FORWARD){
                    ...
                    for (int i = 0; i < 32; i++) {
                        if ( cur_thread_partial_dy[i] > 0) {
                            //Permutation LUTable
                            front_64 |= SPN_DIFF::perm_lookup_global_forward[i][cur_thread_partial_dy[i]][0];
                            back_64 |= SPN_DIFF::perm_lookup_global_forward[i][cur_thread_partial_dy[i]][1];
                        }
                    }
                    #pragma unroll
                    for (int i = 0; i < 16; i++) {
                    ...
                    int save_sbox_index[32]; //Will point to non existance 32 array entry (see substitution below)
                    #pragma unroll
                    for (int i=0;i< 32;i++){
                        save_sbox_index[i] = 32; 
                    }
                    #pragma unroll
                    for (int i = 0; i < 32; i++) {
                    ...
                    if (save_sbox_num <= MAX_AS){  //If only next round AS <= 8
                        //MATSUI BOUND
                        float estimated_com_prob = SPN_DIFF::prob_per_round_remaining_shared[(MAX_ROUND_FORWARD - cur_r - 2)] * SPN_DIFF::prob_per_as_shared[save_sbox_num-1]; 
                        if ((estimated_com_prob * prob_thread) >= SPN_DIFF::CLUSTER_PROB_BOUND_const) {
                        // if (true) {
                            memcpy(output_dx(increment),cur_thread_partial_dy,32);
                ...
                //LAST ROUNDS... no permutation and straight to savings.
                    ...
                    //NOTE: DEBUG: Uncomment this for normal
                    // if (true){
                    if (is_same){
    ...
    while(true){ //Base case, cur_0 and reamining == 0
        ...
        for (int i=0;i<loop_limit;i++){
            ...
            if (dx_ptr < MAX_PATH_PER_ROUND){ //If dx_ptr is within dx_num, [0-N)
                ...
                unsigned char* cur_dx_temp = cur_dx() + ( 32 * dx_ptr ) ;
                int* cur_sbox_index_ptr = cur_sbox_index() + (MAX_AS  * dx_ptr);
                unsigned char cur_thread_partial_dy[33];
                cur_thread_partial_dy[32] = {0};
                memcpy(cur_thread_partial_dy,cur_dx_temp,32);
    ...
    if (threadIdx.x <32){
        if (threadIdx.x<16){
            ...
            for (int j = 0; j < 8; j++) {
                ...
                SPN_DIFF::prob_table_shared[threadIdx.x][j] = SPN_DIFF::freq_table_global_reversed[threadIdx.x][j]/16.0; 
            }
        }
            ...
            if (dx_ptr < MAX_PATH_PER_ROUND){ //If dx_ptr is within dx_num, [0-N)
                ...
                if (cur_r+1 != MAX_ROUND_BACKWARD){
                    ...
                    for (int i = 0; i < 32; i++) {
                        if ( cur_thread_partial_dy[i] > 0) {
                            //Permutation LUTable
                            front_64 |= SPN_DIFF::perm_lookup_global_reversed[i][cur_thread_partial_dy[i]][0];
                            back_64 |= SPN_DIFF::perm_lookup_global_reversed[i][cur_thread_partial_dy[i]][1];
                        }
                    }
                    #pragma unroll
                    for (int i = 0; i < 16; i++) {
                    ...
                    if (save_sbox_num <= MAX_AS){  //If only next round AS <= 8
                        //MATSUI BOUND
                        float estimated_com_prob = SPN_DIFF::prob_per_round_remaining_shared[(MAX_ROUND_BACKWARD - cur_r - 2)] * SPN_DIFF::prob_per_as_shared[save_sbox_num-1]; 
                        if ((estimated_com_prob * prob_thread) >= SPN_DIFF::CLUSTER_PROB_BOUND_const) {
                        // if (true) {
                            memcpy(output_dx(increment),cur_thread_partial_dy,32);
                            *output_prob(increment) = prob_thread;
...
void GPU_Kenerl_t::kernel_compute(int branch_size, unsigned char* dx, unsigned char* dy, int* sbox_index, int* sbox_num, int* nb_size, float* cur_prob, int cur_r, int target_round){
    ...
    cudaStatus = cudaMemcpyToSymbol(SPN_DIFF::final_dy_constant, dy, sizeof(unsigned char)*32);
    if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMemcpyToSymbol final_dy_constant failed!");
    }

    cudaStatus = cudaMemcpy(device_dx, dx, sizeof(unsigned char) * 32 * branch_size, cudaMemcpyHostToDevice);
    if (cudaStatus != cudaSuccess) {
    ...
    *round_offset = cur_r;
    ...
    if (is_MITM){
        ...
        cudaStatus = cudaMemcpy(device_dx, dy, sizeof(unsigned char) * 32 * branch_size, cudaMemcpyHostToDevice);
        if (cudaStatus != cudaSuccess) {
            fprintf(stderr, "cudaMemcpy dx failed!");
        }
        *nb_size = 1;
...
void SPN_DIFF::init(){
    ...
    std::cout <<"\nTrifle Permutation:{\n";
    for (int i = 0; i < 128; i++) {
        SPN_DIFF::perm_host[i] = (i / 4) + ((i % 4) * 32);
    ...
    for (int sbox_pos=0;sbox_pos<32;sbox_pos++){
        for (int sbox_val=0;sbox_val<16;sbox_val++){
			...
			for (int i = 0; i < 32; i++) {
				if (dx[i] > 0) {
					for (int j = 0; j < 4; j++) {
                        ...
                        int bit_pos = (SPN_DIFF::perm_host[((31 - i) * 4) + j]);
                        int bit_pos_reversed = (SPN_DIFF::perm_host_reversed[((31 - i) * 4) + j]);
            ...
            SPN_DIFF::perm_lookup_host[sbox_pos][sbox_val][0]=front_64;
            SPN_DIFF::perm_lookup_host[sbox_pos][sbox_val][1]=back_64;

            SPN_DIFF::perm_lookup_host_reversed[sbox_pos][sbox_val][0]=front_64_reversed;
            SPN_DIFF::perm_lookup_host_reversed[sbox_pos][sbox_val][1]=back_64_reversed;
        }
    }
    std::cout << "}\n" ;
};