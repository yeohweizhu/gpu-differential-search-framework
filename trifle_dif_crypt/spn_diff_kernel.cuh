...
#define MAX_SBOX 32 //32 x 4 = 128bit Sbox
#define BLOCK_SIZE_BIT (MAX_SBOX * 4)

//This can be put in gpu_kernel as variable to let its self manage as well (for different GPU)
#define THREAD_PER_BLOCK 128 //256 //128 //32
#define BLOCK_NUM 72 //GTX 1060 - 72, T4 - , P100 - , V100 -
#define GRID_THREAD_SIZE (THREAD_PER_BLOCK * BLOCK_NUM)
#define MAX_SPACE_PER_THREAD 16 //NOTE: THIS Need to be multiple of 4 because of indexing
...
//MITM Criteria
#define MAX_ROUND_FORWARD 22 //For NON MITM, use MAX_ROUND_FORWARD
#define MAX_ROUND_BACKWARD 21
// #define MAX_ROUND (MAX_ROUND_FORWARD+MAX_ROUND_BACKWARD)

//Pruning Criteria
#define MAX_AS 4
// Cluster Bound is optimized for TRIFLE
const float CLUSTER_PROB_BOUND = (pow(pow(2, -3), MAX_ROUND_FORWARD - 2) * pow(2, -2) * pow(2, -2) * pow(2, -23)); 
const float CLUSTER_PROB_BOUND_FORWARD = (pow(pow(2, -3), MAX_ROUND_FORWARD - 2) * pow(2, -2) * pow(2, -2) * pow(2, -23)); 
const float CLUSTER_PROB_BOUND_BACKWARD = (pow(pow(2, -3), MAX_ROUND_BACKWARD - 2) * pow(2, -2) * pow(2, -2) * pow(2, -23)); 
// const float CLUSTER_PROB_BOUND_PURE = (pow(pzow(2, -3), MAX_ROUND - 2) * pow(2, -2) * pow(2, -2) * pow(2, -21)); 
...
namespace SPN_DIFF{
	...
	*/
    ...
    extern unsigned long long perm_lookup_host[MAX_SBOX][16][2];
    extern unsigned long long perm_lookup_host_reversed[MAX_SBOX][16][2];

    extern unsigned int diff_table_host[][8]; //There can ever be 8 maximum diff pattern
